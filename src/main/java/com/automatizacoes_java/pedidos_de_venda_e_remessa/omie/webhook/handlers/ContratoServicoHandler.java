...
@Component
public class ContratoServicoHandler implements OmieWebhookHandler {
	private static final Logger logger = LoggerFactory.getLogger(ContratoServicoHandler.class);

	private final ObjectMapper objectMapper = new ObjectMapper();

	private final Map<String, OmieWebhookActionStrategy<ContratoServicoCadastroDTO>> actionStrategies;

	public ContratoServicoHandler(List<OmieWebhookActionStrategy<ContratoServicoCadastroDTO>> strategies) {
	...
	public void processar(JsonNode event, EmpresaEntity empresa, String acao) {
		logger.info("Webhook da OMIE recebido para Contrato  Servico Cadastro");
		logger.info("Empresa: {}", empresa);
		logger.info("Acao: {}", acao);
		logger.info("Valor: {}", event);

		try {
			ContratoServicoCadastroDTO dto = objectMapper.treeToValue(event, ContratoServicoCadastroDTO.class);
