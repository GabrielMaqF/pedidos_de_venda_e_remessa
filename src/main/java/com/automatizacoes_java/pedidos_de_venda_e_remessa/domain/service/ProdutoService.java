...
import com.automatizacoes_java.pedidos_de_venda_e_remessa.domain.repository.ProdutoRepository;
...
@Service
public class ProdutoService extends BaseService<ProdutoEntity, EntidadeCompostaId, Long> {
	...
	@Transactional
	public ProdutoEntity criarOuAtualizarPorOmie(ProdutoDTO dto, EmpresaEntity empresa) {
		EntidadeCompostaId id = new EntidadeCompostaId(String.valueOf(dto.getCodigoProduto()), empresa.getCodigo());

		// Procura o cliente, se não existir, cria um novo
		ProdutoEntity entidade = repository.findById(id).orElse(new ProdutoEntity(dto, empresa));
	...
	public CompletableFuture<ResponseEntity<?>> getAllOmieUpdateBDA(List<EmpresaEntity> empresas) {
		...
		for (EmpresaEntity e : empresas) {
			...
			try {
				Thread.sleep(1000L);
				do {
					OmieListarProdutoResponse res = omieApiClientService.listarProdutoPorPagina(e, paginaAtual).get();

					if (res == null || res.getProdutoServicoCadastro().isEmpty())
						break;

					totalPaginas = res.getTotalDePaginas();

					for (ProdutoDTO dto : res.getProdutoServicoCadastro()) {
